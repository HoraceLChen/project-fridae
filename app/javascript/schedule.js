...
// var totalMinutes = (+minTime[0]) * 60 + (+minTime[1]);
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  setInterval(() => {
    ...
    const totalMinutes = (+minTime[0]) * 60 + (+minTime[1]);
    // calculate the relative position of the needle
    const relativePosition = totalMinutes / 1440;

    // get the height of the rectangle
    const scheduleHeight = document.querySelector(".schedule-box").clientHeight;
    // calculate the position of the needle
    const needlePosition = relativePosition * scheduleHeight;

    // move the needle
    document.querySelector(".needle").style.top = `${needlePosition}px`;
  }, 1000);
});
